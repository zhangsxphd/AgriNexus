...
import { nowIso, todayDateString, parseJson, formatDateTime, insertActivityLog } from '../helpers/shared.js';
...
const ALLOWED_ID_TABLES = new Set([
  'greenhouses', 'sensor_readings', 'control_logs', 'irrigation_events',
  'activity_logs', 'config_snapshots', 'alerts', 'decisions',
]);

function getNextId(db, tableName) {
  if (!ALLOWED_ID_TABLES.has(tableName)) {
    throw new Error(`getNextId: table "${tableName}" is not in the allow-list`);
  }
...
export function getDashboardBundle() {
  ...
  // 只查询首页展示需要的 3 条报警，而非加载全部再 filter
  const alerts = db
    .prepare(`
      SELECT a.*, g.name AS greenhouse_name, ru.name AS resolved_by_name
      FROM alerts a
  ...
  // 只查询首页展示需要的 3 个棚室，而非加载全部
  const greenhouses = listGreenhousesBase(db)
    .filter((row) => [1, 2, 3].includes(row.id))
    .map(toGreenhouseSummary);
